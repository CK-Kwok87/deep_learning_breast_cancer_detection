...
### Methodology
- **Preprocessing Correction:** Moved the `rescale=1./255` operation to the *end* of the custom CLAHE function. This ensures CLAHE operates on the original 0-255 pixel intensities.
...
## Planned Roadmap (Future Iterations)

### Iteration 8: Background Masking & Memory-Safe Augmentation
**Status:** Configured in the notebook; results pending execution and evaluation.
**Objective:** Extend the successful **VGG16 + 320x320** pipeline with explicit background masking and medically plausible augmentation, while keeping the experiment runnable on a **4 GB GPU**.

### Methodology
Iteration 8 keeps the high-resolution **VGG16 + 320x320** backbone from Iteration 7 and adds explicit **background masking** before CLAHE enhancement. The preprocessing pipeline thresholds the mammogram, extracts the largest breast contour, and suppresses non-breast regions so the model is less likely to rely on scanner borders, padding, or other background artifacts.

To preserve mammographic realism, the originally planned aggressive augmentation strategy was replaced with a **safe augmentation policy**:
...
The training configuration was also adjusted for hardware constraints. **Batch size was reduced to 1**, the classifier head was simplified to **GlobalAveragePooling + Dense(128) + Dropout + Dense(1)**, and fine-tuning was restricted to **VGG block 5 only** during Stage 2. Full-backbone Stage 3 fine-tuning was disabled by default to avoid out-of-memory errors on the local **GTX 1660 SUPER (4 GB VRAM)**.

### Expected Impact
Iteration 8 is designed to directly address the shortcutting issue identified in Iteration 6 by forcing the model to focus on breast tissue rather than irrelevant background structure. At the same time, the conservative augmentation policy should improve robustness without introducing anatomically implausible transformations. The memory-safe training strategy preserves the key benefit of the high-resolution VGG pipeline while making the experiment practical on the available hardware.

### Iteration 9: Final Ensemble & Decision Fusion
**Objective:** Combine the best-performing models into a weighted ensemble.
**Rationale:** Leverage complementary architecture strengths to maximize clinical reliability.
...
### Dataset & Task
- **Sawyer-Lee, R., et al. (2017).** "Curated Breast Imaging Subset of Digital Database for Screening Mammography." *Scientific Data*, 4(1). [CBIS-DDSM Foundation]
- **Yagis, E., et al. (2021).** "Data leakage in machine learning-based medical image analysis." *Scientific Reports*, 11(1). [Justification for Patient-Level Splitting]

### Architecture & Optimization
- **Tan, M., & Le, Q. (2019).** "EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks." *ICML*. [EfficientNet Foundation]
- **Morid, M. A., et al. (2021).** "A scoping review of transfer learning research on medical image analysis using ImageNet." *Computers in Biology and Medicine*. [Transfer Learning Methodology]

### Preprocessing
- **Zuiderveld, K. (1994).** "Contrast Limited Adaptive Histogram Equalization." *Graphics Gems IV*. [Original CLAHE Algorithm]
- **Carneiro, P. C., et al. (2019).** "CLAHE Parameters Effects on the Quantitative and Visual Assessment of Dense Breast Mammograms." *IEEE Latin America Transactions*. [Application to Mammography]
